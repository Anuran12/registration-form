...
function checkReceipt(submissionId) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(
      () => finish(reject, new Error('The registration service did not confirm the save in time.')),
      20000,
    );

    globalThis[callbackName] = (data) => finish(resolve, data);
    script.onerror = () => finish(reject, new Error('The registration service could not be reached.'));
...
export async function submitRegistration(payload) {
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    ...
    for (let statusAttempt = 1; statusAttempt <= STATUS_CHECKS_PER_ATTEMPT; statusAttempt += 1) {
      await delay(500 * statusAttempt);
      try {
        const data = await checkReceipt(payload.submissionId);
        if (!data.ok) throw new Error(data.message || 'Registration could not be saved.');
  ...
  throw lastError || new Error('Unable to confirm that the registration was saved. Please try again.');
}
